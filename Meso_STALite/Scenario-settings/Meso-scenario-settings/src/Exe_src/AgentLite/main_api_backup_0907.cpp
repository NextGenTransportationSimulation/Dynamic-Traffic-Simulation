...
//	fprintf(g_pFileOutputLog, "number of zones =,%lu\n", g_zone_vector.size());
	...
	CCSVParser parser;
	log_out << "Step 4: Reading file section [demand_file_list]..." << endl;
		...
		while (parser.ReadRecord_Section())
		{
			...
			if (format_type.find("column") != string::npos)  // or muliti-column
			{
				...
				if (st != NULL)
				{
					...
					log_out << "total_demand_volume is " << assignment.total_demand_volume << endl;
				}
				else  //open file
				{
...
void g_ReadInputData(Assignment& assignment)
{
	...
	log_out << "Step 1.1: Reading section [demand_period]..." << endl;
	...
	log_out << "Reading section [link_type]..." << endl;
	...
	log_out << "Step 2: Reading section [agent_type]..." << endl;
	...
	if (parser.OpenCSVFile("node.csv", true))
	{
		...
		while (parser.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
		{
			...
			{ 
				...
				{
					log_out << "warning: zone_id " << zone_id << " have been defined more than once." << endl;
	...
	for (int i = 0; i < g_node_vector.size(); i++)
	{
		...
		{ // we need to make sure we only create a zone in the memory if only there is positive demand flow from the (new) OD table
			...
			g_zone_vector.push_back(ozone);  // add element into vector
											 //	log_out << ozone.zone_id << ' ' << ozone.zone_seq_no << endl;
		}
	}
...
void g_reload_service_arc_data(Assignment& assignment)
{
		...
		if (parser_service_arc.OpenCSVFile("service_arc.csv", false))
		{
			while (parser_service_arc.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
			{
				...
				{
					log_out << "Error: from_node_id in file service_arc.csv is not defined." << endl;
					continue;
				}
				...
				if (assignment.g_internal_node_to_seq_no_map.find(from_node_id) == assignment.g_internal_node_to_seq_no_map.end())
				{
					log_out << "Error: from_node_id " << from_node_id << " in file service_arc.csv is not defined in node.csv." << endl;

					continue; //has not been defined
				}
				if (assignment.g_internal_node_to_seq_no_map.find(to_node_id) == assignment.g_internal_node_to_seq_no_map.end())
				{
					log_out << "Error: to_node_id " << to_node_id << " in file service_arc.csv is not defined in node.csv." << endl;
					continue; //has not been defined
				}
				...
				{
					log_out << "Error: Link " << from_node_id << "->" << to_node_id << " in file service_arc.csv is not defined in link.csv." << endl;
					continue;
				}
				...
				{
					log_out << "Error: Field time_window in file service_arc.csv cannot be read." << endl;
					g_ProgramStop();
					break;
				}
				...
				if (assignment.g_number_of_service_arcs % 10000 == 0)
					log_out << "reading " << assignment.g_number_of_service_arcs << " service_arcs.. " << endl;
			}

			parser_service_arc.CloseCSVFile();
		}
	// we now know the number of links
	log_out << "number of service_arcs = " << assignment.g_number_of_service_arcs << endl;
...
void g_column_pool_optimization(Assignment& assignment, int column_updating_iterations)
{
	...
	{ 
		log_out << "column pool updating: no.  " << n << endl;
...
double network_assignment(int iteration_number, int assignment_mode, int column_updating_iterations, int signal_updating_iterations)
{
	...
	//{
	//	log_out << "File output_solution.csv cannot be opened." << endl;
	//	g_ProgramStop();
	//}
	...
	for (int iteration_number = 0; iteration_number < assignment.g_number_of_K_paths; iteration_number++)
	{
		...
		end_t = clock();
		iteration_t = end_t - start_t;
		log_out << "assignment iteration = " << iteration_number << " with CPU time " << iteration_t / 1000.0 << " s" << endl;
...
//		log_out << "column generation with CPU time " << cumulative_cp / 1000.0 << " s; " << endl;
	...
	log_out << "column pool updating with " << column_updating_iterations << " iterations" << endl;
	start_t = clock();
	...
	end_t = clock();
	total_t = (end_t - start_t);
	log_out << "Output for assignment with " << assignment.g_number_of_K_paths << " iterations. Traffic assignment completes!" << endl;
