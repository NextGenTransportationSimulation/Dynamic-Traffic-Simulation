...
def max_cong_period(period, vdf_name, User_Time_period, Congest_value_all, Interval_value_all):
    if period == User_Time_period[0]:
        if vdf_name[0] == 1:
            return Congest_value_all[0]
        else:
            return Interval_value_all[0]
    if period == User_Time_period[1]:
        if vdf_name[0] == 1:
            return Congest_value_all[1]
        else:
            return Interval_value_all[1]
    if period == User_Time_period[2]:
        if vdf_name[0] == 1:
            return Congest_value_all[2]
        else:
            return Interval_value_all[2]
    if period == User_Time_period[3]:
        if vdf_name[0] == 1:
            return Congest_value_all[3]
        else:
            return Interval_value_all[3]
    if period == "Day":
        if vdf_name[0] == 1:
            return 5
        else:
...
def input_data():
    ...
    # Filtering
    # data_df = data_df[(data_df['AT'] == 1) & (data_df['FT'] == 7)]
...
def vdf_calculation(internal_vdf_dlink_df, vdf_name, period_name, CUT_OFF_SPD, ULT_CAP, K_CRI, FFS, mm,
                    peak_factor_avg, User_Time_period, Congest_value_all, Interval_value_all):
    ...
    if METHOD == 'VBM':
        ...
        for k in range(0, len(internal_vdf_dlink_df)):
            ...
            for kk in range(weight_max_cong_period):
                Y_data.append(0.001)
                X_data.append(
                    max_cong_period(period_name, vdf_name, User_Time_period, Congest_value_all, Interval_value_all))
...
def vdf_calculation_daily(temp_daily_df, vdf_name, CUT_OFF_SPD, ULT_CAP, K_CRI, FFS, mm, User_Time_period,
                          Congest_value_all, Interval_value_all):
    p0 = np.array([FFS, 0.15, 4])
    ...
    for k in range(0, len(temp_daily_df)):
        ...
        for kk in range(weight_max_cong_period):
            Y_data.append(0.001)
            X_data.append(max_cong_period("Day", vdf_name, User_Time_period, Congest_value_all, Interval_value_all))
        x = np.array(X_data)
        y = np.array(Y_data)
...
def nb_sample_checking(period, User_Time_period, Congest_value_all, Interval_value_all):
    if period == User_Time_period[0]:
        return Interval_value_all[0] * (60 / TIM_STAMP)
    if period == User_Time_period[1]:
        return Interval_value_all[1] * (60 / TIM_STAMP)
    if period == User_Time_period[2]:
        return Interval_value_all[2] * (60 / TIM_STAMP)
    if period == User_Time_period[3]:
        return Interval_value_all[3] * (60 / TIM_STAMP)
...
if __name__ == "__main__":
    ...
    User_Time_period = list(set(training_set['assignment_period']))
    # Calculate the length of period and max congest period
    Interval_value_all = []
    Congest_value_all = []
    for jj in User_Time_period:
        time_ss = [int(var[0:2]) for var in jj.split('_')]
        if time_ss[0] > time_ss[1]:
            Interval_value = time_ss[1] + 24 - time_ss[0]
            Congest_value = 23 - time_ss[0]
        else:
            Interval_value = time_ss[1] - time_ss[0]
            Congest_value = time_ss[1] - time_ss[0]
        Interval_value_all.append(Interval_value)
        Congest_value_all.append(Congest_value)
    ...
    for vdf_name, vdf_trainingset in vdf_group:
        ...
        for period_name, pvdf_trainset in pvdf_group:
            ...
            for dlink_name, dlink_training_set in dlink_group:
                ...
                if len(dlink_training_set) < nb_sample_checking(period_name, User_Time_period, Congest_value_all,
                                                                Interval_value_all):
                    print('WARNING:  link ', dlink_id, 'in period', period_name,
                          'does not have all 15 minutes records...')
                    print((1 - len(dlink_training_set) / nb_sample_checking(period_name, User_Time_period,
                                                                            Congest_value_all,
                                                                            Interval_value_all)) * 100,
                          '% of records of the link of the time period are missing...\n')
                    if (1 - len(dlink_training_set) / nb_sample_checking(period_name, User_Time_period,
                                                                         Congest_value_all,
                                                                         Interval_value_all)) >= INCOMP_SAMPLE:
                        continue
            ...
            if len(vdf_dlink_list) == 0:
                print('No Road Link have enough data')
                break
            internal_vdf_dlink_df = pd.DataFrame(vdf_dlink_list)
            internal_vdf_dlink_df.columns = \
                ['link_id', 'from_node_id', 'to_node_id', 'Date', 'FT', 'AT', 'period', 'vol_period',
                 'vol_period_hourly', 'speed_period', 'density_period', 't0', 't3', 'Demand', 'avg_discharge_rate',
                 'peak_factor', 'congestion_period', 'rho', 'waiting_time']
            ...
            calibration_vdf_dlink_results = vdf_calculation(internal_vdf_dlink_df, vdf_name, period_name, CUT_OFF_SPD,
                                                            ULT_CAP, K_CRI, FFS, mm, peak_factor_avg, User_Time_period,
                                                            Congest_value_all, Interval_value_all)
            ...
            internal_vdf_link_df.columns = \
                ['link_id', 'from_node_id', 'to_node_id', 'FT', 'AT', 'period', 'vol_period',
                 'vol_period_hourly', 'speed_period', 'density_period', 't0', 't3', 'Demand', 'VOC', 'VOC_period',
                 'alpha', 'beta', 'peak_factor', 'period_cap', 'vol_valid', 'demand_est']
    ...
    if len(g_parameter_list) > 0:
        para_df = pd.DataFrame(g_parameter_list,
                               columns=['VDF', 'VDF_TYPE', 'FT', 'AT', 'period', 'CUT_OFF_SPD', 'ULT_CAP', 'K_CRI',
                                        'FFS', 'mm', 'peak_factor', 'alpha', 'beta', 'per_error', 'per_error_demand'])
        ...
        para_df_am = para_df[para_df.period == User_Time_period[0]]
        ...
        time_period = 'MD'
        cap_period = time_period + '_CAP'
        para_df_md = para_df[para_df.period == User_Time_period[1]]
        ...
        time_period = 'PM'
        cap_period = time_period + '_CAP'
        para_df_pm = para_df[para_df.period == User_Time_period[2]]
        ...
        time_period = 'NT'
        cap_period = time_period + '_CAP'
        para_df_nt = para_df[para_df.period == User_Time_period[3]]
        ...
        pk_spd_cap['SPEED'] = pk_spd_cap.apply(
            lambda x: speed_dict[int(x['VDF_TYPE'])] if int(x['VDF_TYPE']) in am_cap_dict.keys() else x['SPEED'],
            axis=1)
        ...
        if OUTPUT == "DAY":
            ...
            pk_spd_cap['ALPHA'] = pk_spd_cap.apply(
                ...
                axis=1)
            pk_spd_cap['BETA'] = pk_spd_cap.apply(
                lambda x: beta_dict[int(x['VDF_TYPE'])] if int(x['VDF_TYPE']) in am_cap_dict.keys() else x['BETA'],
                axis=1)
            pk_spd_cap.to_csv('./1_calibration_output/4_p k_spd_cap_day.csv', index=False)
